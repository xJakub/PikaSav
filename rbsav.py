...
class RBSav():
    def __init__(self,file,repair=False):
        ...
        self.ok = repair
        self.repair = repair
        self.maps = [("Main",0xd2ec,0x2598,0x3522)]
        table = [""]*256;
        etable = [""]*256;
        for x in range(26):
    ...
    def check_sav(self):
        ...
        size = 0x0f8b
        checksum = 0xFF
        self.checksum = ord(self.buffer[0x3523])
        ...
        self.setbyte(0x3523,checksum)
    
    
    def set(self,var,value):
        if (var=="name"):
            value=self.encode(value,7)
            self.buffer = self.buffer[0:0x2598] + value + chr(80) + chr(80) + chr(80) + chr(80) + self.buffer[0x25a3:]
        if (var=="rivalname"):
            value=self.encode(value,7)
            self.buffer = self.buffer[0:0x25f6] + value + chr(80) + chr(80) + chr(80) + chr(80) + self.buffer[0x2601:]
        if (var=="money"):
            value = value[-6:].rjust(6,"0")
            self.setbyte(0x25f3,int(value[0:2],16))
            self.setbyte(0x25f4,int(value[2:4],16))
            self.setbyte(0x25f5,int(value[4:6],16))
        if (var=="chips"):
            value = value[-4:].rjust(4,"0")
            self.setbyte(0x2850,int(value[0:2],16))
            self.setbyte(0x2851,int(value[2:4],16))
        if (var=="hours"):
            self.setbyte(0x2ced,int(value) & 255)
            self.setbyte(0x2cee,int(value) >> 8)
        if (var=="minutes"):
            self.setbyte(0x2cef,int(value))
        if (var=="seconds"):
            self.setbyte(0x2cf0,int(value))
        if (var=="itemcount"):
            self.setbyte(0x25c9,int(value))
        if (var=="pcitemcount"):
            self.setbyte(0x27e6,int(value))
        if (var=="pokemoncount"):
            self.setbyte(0x2f2c,int(value))
        for b in range(12):
    ...
    def load_time(self):
        self.hours = ord(self.buffer[0x2ced])
        self.hours += ord(self.buffer[0x2cee])*256
        self.minutes = ord(self.buffer[0x2cef])
        self.seconds = ord(self.buffer[0x2cf0])
    ...
    def setpokedex(self,x,isseen,iscatched):
        pos = 2 ** ((x-1)%8)
        seen = ord(self.buffer[0x25a3+(x-1)//8])
        catched = ord(self.buffer[0x25b6+(x-1)//8])
        if self.seen[x] != isseen:
            seen ^= pos
        if self.catched[x] != iscatched:
            catched ^= pos
        self.setbyte(0x25b6+(x-1)//8,seen)
        self.setbyte(0x25a3+(x-1)//8,catched)
    ...
    def load_pokedex(self):
        ...
        for x in range(19):
            catched = ord(self.buffer[0x25a3+x])
            seen = ord(self.buffer[0x25b6+x])
            for y in range(8):
    ...
    def load_items(self):
        items = [[0,0]]*50
        for x in range(50):
            item = ord(self.buffer[0x25ca+2*x]);
            count = ord(self.buffer[0x25cb+2*x]);
            items[x]=[item,count];
        self.items = items
        self.itemcount = ord(self.buffer[0x25c9])
        
        pcitems = [[0,0]]*50
        for x in range(50):
            item = ord(self.buffer[0x27e7+2*x]);
            count = ord(self.buffer[0x27e8+2*x]);
            pcitems[x]=[item,count];
        self.pcitems = pcitems
        self.pcitemcount = ord(self.buffer[0x27e6])


    def load_pokemon(self):
        self.pokemon = [""]*6
        self.pcpokemon = [""]*240
        self.pokemoncount = ord(self.buffer[0x2f2c])
        self.boxpokemoncount = [0]*12
        self.boxoffset = [0]*12
        for p in range(6):
            self.pokemon[p] = self.pkm(0x2f2d+p,0x303c+11*p,0x307e+11*p,0x2f34+44*p)
        self.currentbox = ord(self.buffer[0x284c]) & 127
        
        for b in range(12):
            offset = 0x4000 + ((b//6) * 0x2000) + 0x462*(b%6)
    ...
    def setpokemon(self,p,pkm):
        self.pkm(0x2f2d+p,0x303c+11*p,0x307e+11*p,0x2f34+44*p,pkm)
    ...
    def pkm(self,off_hex,off_otname,off_name,off_data,data=None):
        if data == None:
            ...
            pkm += self.buffer[off_data:off_data+44] #23/0x17 - 66/0x42
            return pkm
        else:
    ...
    def pcpkm(self,off_hex,off_otname,off_name,off_data,data=None):
        if data == None:
            ...
            pkm += self.buffer[off_otname:off_otname+11] #1/0x01 - #11/0x0b
            pkm += self.buffer[off_name:off_name+11] #12/0x0c - 22/0x16
            pkm += self.buffer[off_data:off_data+33] #23/0x17 - 55/0x37
            return pkm
        else:
            self.setbyte(off_hex,ord(data[0]))
            self.buffer = self.buffer[0:off_otname]+data[1:12]+self.buffer[off_otname+11:]
    ...
    def pkm_get(self,pkm,var):
        ...
        if var == "level":
            return ord(pkm[26])
        if var == "asleep":
            if ord(pkm[27]) & 7: return True
            return False
        if var == "poisoned":
            if ord(pkm[27]) & 8: return True
            return False
        if var == "burned":
            if ord(pkm[27]) & 16: return True
            return False
        if var == "frozen":
            if ord(pkm[27]) & 32: return True
            return False
        if var == "paralyzed":
            if ord(pkm[27]) & 64: return True
            return False
        if var == "ok":
            if ord(pkm[27]) & 127: return False
            return True
        if var == "type1":
            return ord(pkm[28])
        if var == "type2":
    ...
    def setitem(self,x,item,count):
        if (x<20):
            self.setbyte(0x25ca+2*x,item)
            self.setbyte(0x25cb+2*x,count)
        if (x>=20):
            self.setbyte(0x27e7+2*(x-20),item)
            self.setbyte(0x27e8+2*(x-20),count)
            
    def load_money(self):
        money = ""
        money += hex(ord(self.buffer[0x25f3]))[2:].rjust(2,"0")
        money += hex(ord(self.buffer[0x25f4]))[2:].rjust(2,"0")
        money += hex(ord(self.buffer[0x25f5]))[2:].rjust(2,"0")
        chips = hex(ord(self.buffer[0x2850]))[2:].rjust(2,"0")
        chips += hex(ord(self.buffer[0x2851]))[2:].rjust(2,"0")
    ...
    def load_names(self):
        self.name = self.decode(self.buffer[0x2598:0x25a2])
        self.rivalname = self.decode(self.buffer[0x25f6:0x2600])
