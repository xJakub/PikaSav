...
class GSSav:
    ...
    def check_sav(self):
        ...
        size = 3424
        checksum = 0
        self.checksum = ord(self.buffer[11625]) + ord(self.buffer[11626]) * 256
        for x in range(size):
    ...
    def load_time(self):
        self.hours = ord(self.buffer[8276])
        self.hours += ord(self.buffer[8275]) * 256
        self.minutes = ord(self.buffer[8277])
        self.seconds = ord(self.buffer[8279])
    ...
    def setpokedex(self, x, isseen, iscatched):
        pos = 2 ** ((x - 1) % 8)
        seen = ord(self.buffer[10828 + (x - 1) // 8])
        catched = ord(self.buffer[10860 + (x - 1) // 8])
    ...
    def load_pokedex(self):
        ...
        self.catched = [0] * 257
        for x in range(32):
            catched = ord(self.buffer[10828 + x])
            seen = ord(self.buffer[10860 + x])
            for y in range(8):
    ...
    def load_items(self):
        items = [[0, 0]] * 20
        for x in range(20):
            item = ord(self.buffer[9248 + 2 * x])
            count = ord(self.buffer[9249 + 2 * x])
            items[x] = [item, count]

        self.items = items
        self.itemcount = ord(self.buffer[9247])
        pcitems = [[0, 0]] * 50
        for x in range(50):
            item = ord(self.buffer[9343 + 2 * x])
            count = ord(self.buffer[9344 + 2 * x])
            pcitems[x] = [item, count]

        self.pcitems = pcitems
        self.pcitemcount = ord(self.buffer[9342])

    def load_pokemon(self):
        self.pokemon = [''] * 6
        self.pcpokemon = [''] * 280
        self.pokemoncount = ord(self.buffer[10378])
        self.boxpokemoncount = [0] * 14
        self.boxoffset = [0] * 14
        for p in range(6):
            self.pokemon[p] = self.pkm(10379 + p, 10674 + 11 * p, 10740 + 11 * p, 10386 + p * 48)

        self.currentbox = ord(self.buffer[10020]) & 127
        for b in range(14):
    ...
    def load_money(self):
        money = 0
        money += ord(self.buffer[9179]) * 65536
        money += ord(self.buffer[9180]) * 256
        money += ord(self.buffer[9181])
        chips = 0
        chips += ord(self.buffer[9186]) * 256
        chips += ord(self.buffer[9187])
        self.money = money
        self.chips = chips
