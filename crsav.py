...
class CRSav():
    ...
    def check_sav(self):
        ...
        size = 0xB7a
        checksum = 0
        self.checksum = ord(self.buffer[0x2d0d])+ord(self.buffer[0x2d0e])*256
    ...
    def set(self,var,value):
        ...
        if (var=="money"):
            self.setbyte(0x23dc,(int(value) >> 16) & 255)
            self.setbyte(0x23dd,(int(value) >> 8) & 255)
            self.setbyte(0x23de,int(value) & 255)
        if (var=="chips"):
            self.setbyte(0x23e3,int(value) >> 8)
            self.setbyte(0x23e4,int(value) & 255)
        if (var=="hours"):
            self.setbyte(0x2053,int(value) & 255)
            self.setbyte(0x2052,int(value) >> 8)
        if (var=="minutes"):
            self.setbyte(0x2054,int(value))
        if (var=="seconds"):
    ...
    def load_time(self):
        self.hours = ord(self.buffer[0x2053])
        self.hours += ord(self.buffer[0x2052])*256
        self.minutes = ord(self.buffer[0x2054])
        self.seconds = ord(self.buffer[0x2055])
    ...
    def setpokedex(self,x,isseen,iscatched):
        pos = 2 ** ((x-1)%8)
        seen = ord(self.buffer[0x2a27+(x-1)//8])
        catched = ord(self.buffer[0x2a47+(x-1)//8])
        if self.seen[x] != isseen:
            seen ^= pos
        if self.catched[x] != iscatched:
            catched ^= pos
        self.setbyte(0x2a27+(x-1)//8,seen)
        self.setbyte(0x2a47+(x-1)//8,catched)
    ...
    def load_pokedex(self):
        ...
        for x in range(32):
            catched = ord(self.buffer[0x2a27+x])
            seen = ord(self.buffer[0x2a47+x])
            for y in range(8):
    ...
    def load_items(self):
        items = [[0,0]]*20
        for x in range(20):
            item = ord(self.buffer[0x2421+2*x]);
            count = ord(self.buffer[0x2422+2*x]);
            items[x]=[item,count];
        self.items = items
        self.itemcount = ord(self.buffer[0x2420])
        
        pcitems = [[0,0]]*50
        for x in range(50):
            item = ord(self.buffer[0x2480+2*x]);
            count = ord(self.buffer[0x2481+2*x]);
    ...
    def load_pokemon(self):
        self.pokemon = [""]*6
        self.pcpokemon = [""]*280
        self.pokemoncount = ord(self.buffer[0x2865])
        self.boxpokemoncount = [0]*14
        self.boxoffset = [0]*14
        for p in range(6):
            self.pokemon[p] = self.pkm(0x2865+1+p,0x2865+296+11*p,0x2865+362+11*p,0x2865+8+p*48)
        self.currentbox = ord(self.buffer[0x2700]) & 127
    ...
    def setitem(self,x,item,count):
        if (x<20):
            self.setbyte(0x2421+2*x,item)
            self.setbyte(0x2422+2*x,count)
        if (x>=20):            
            self.setbyte(0x2480+2*(x-20),item)
            self.setbyte(0x2481+2*(x-20),count)
            
    def load_money(self):
        money = 0
        money += ord(self.buffer[0x23dc])*65536
        money += ord(self.buffer[0x23dd])*256
        money += ord(self.buffer[0x23de])
        chips = 0
        chips += ord(self.buffer[0x23e3])*256
        chips += ord(self.buffer[0x23e4])
